            ...
            <div class="card-body">
                <form action="{{route('admin.gallery.store')}}" method="post" enctype="multipart/form-data" >
                    ...
                    <div class="text-right">
                        <button type="submit" class="btn btn-primary">Add Now <i class="icon-paperplane ml-2"></i></button>
                    </div>
...
<div class="card">
    ...
    <table class="table datatable-save-state">
        ...
        <tbody>
            @foreach (App\Models\Gallery::all() as $key => $image)
            <tr>
                <td>{{$key+1}}</td>
                <td><img src="{{asset($image->image)}}" style="width:100px;height:auto;"></td>
                <td>
                    <button data-toggle="modal" data-target="#edit_modal"
                    id="{{$image->id}}" class="edit-btn btn btn-primary">Edit</button>
                </td>
                <td>
        ...
        <form id="updateForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myModalLabel">Update Gallery Image</h5>
...
<script>
    $(document).ready(function(){
        $('.edit-btn').click(function(){
            let id = $(this).attr('id');
            $('#id').val(id);
            $('#updateForm').attr('action','{{route('admin.gallery.update','')}}' +'/'+id);
        });
    });
</script>
